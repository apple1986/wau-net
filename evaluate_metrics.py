...
def parse_args():
    ...
    parser.add_argument("--prob-dir", type=str, default=None,
                        help="Optional foreground-probability map directory for accurate AUC.")
    parser.add_argument(
        "--pred-dirs",
        nargs="*",
        default=None,
...
def find_prediction_file(pred_dir, gt_path):
    ...
    # test.py saves binary predictions as <stem>_grey.png. Prefer that form.
    for ext in exts:
        p = pred_dir / f"{stem}_grey{ext}"
        if p.exists():
            return p

    # Also support externally generated predictions that preserve the GT name.
    ...
    # Recursive fallback, explicitly excluding saved GT/probability files.
    candidates = []
    for f in list_image_files(pred_dir):
        if 'probability' in f.parts or f.stem.endswith('_gt') or f.stem.endswith('_prob'):
            continue
        if f.stem == stem or f.stem == f"{stem}_grey":
            candidates.append(f)
...
def load_pred(pred_path, img_size, threshold, prob_path=None):
    ...
    if prob_path is not None:
        ...
        prob_img = prob_img.resize((img_size, img_size), Image.BILINEAR)
        pred_score = np.array(prob_img).astype(np.float32) / 255.0
    elif arr.max() > arr.min():
        # Fallback for legacy binary-mask-only results.
...
def evaluate_model(model_name, pred_dir, gt_files, img_size, threshold, prob_dir=None):
    rows = []
    all_scores = []
    all_gts = []
    ...
    for gt_path in tqdm(gt_files, desc=f"Evaluating {model_name}"):
        ...
        if prob_dir is not None:
            prob_dir_path = Path(prob_dir)
            candidates = list(prob_dir_path.glob(f"{gt_path.stem}_prob.*"))
            if candidates:
                prob_path = candidates[0]
        pred, score = load_pred(pred_path, img_size, threshold, prob_path=prob_path)
...
def main():
    ...
    for model_name, pred_dir in pred_dirs.items():
        ...
        rows, global_auc = evaluate_model(
            ...
            prob_dir=args.prob_dir if len(pred_dirs) == 1 else None,
        )
