...
class Shallow_net(nn.Module):
    def __init__(self, dw_channels1=32, dw_channels2=64, out_channels=128, **kwargs):
        ...
        self.conv = Custom_Conv(1, dw_channels1, 3, 2)  # 输入通道改为 1
...
class Deep_net(nn.Module):
    def __init__(self, in_channels, block_channels, t, num_blocks, **kwargs):
        ...
        self.conv_ = Custom_Conv(1, in_channels, 3, 2)  # 输入通道改为 1
...
class ContextNet(nn.Module):
    def __init__(self, classes=2, aux=False, **kwargs):  # 默认类别数改为 2
...
if __name__ == '__main__':
    ...
    model = ContextNet(classes=2).to(device)  # 设置类别数为 2
    summary(model, (1, 256, 256))  # 输入调整为单通道 256x256