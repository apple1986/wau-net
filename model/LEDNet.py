...
class LEDNet(nn.Module):
    def __init__(self, classes):
        super().__init__()
        self.initial_block = DownsamplerBlock(1, 32)  # Input channels changed to 1
        ...
        self.apn = APNModule(in_ch=128, out_ch=classes)  # Output channels = classes

    def forward(self, input):
        output = self.initial_block(input)  # 256x256 -> 128x128
        for layer in self.layers:
            output = layer(output)
        output = self.apn(output)  # 32x32 -> 256x256 (via APN and interpolation)
        out = F.interpolate(output, input.size()[2:], mode="bilinear", align_corners=True)
        return out

if __name__ == '__main__':
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = LEDNet(classes=2).to(device)  # 2 classes for binary segmentation
